...
**M2 — Pixels and inputs.** *In progress (2026-08-05): first debug APK builds in CI, and the APK **boots homebrew in CI on every push*** — `libcellstation.so` (JNI bridge + EmuCallbacks over the unmodified core, Vulkan surface via `ANativeWindow`, pad_thread, Cubeb/AAudio, lean PUP installer, logcat logging) packaged by a minimal Kotlin app (firmware install, game list from app-external storage, fullscreen `EmulationActivity` already exposing the public `EMULATE` intent). **CI boot test green (2026-08-05)**: the emulator lane (API 30 x86_64 image, arm64 via `ndk_translation`, armv8-a compat build) installs the APK, fires the `EMULATE` intent at `gs_gcm_tetris.elf` (RPCS3's own GPL test homebrew), and asserts `Boot OK` + RSX activity in logcat; a native Linux host harness (`native/host/`) boots the same payload for local iteration. Firmware-free boots force-HLE all firmware modules (auto-reverted when a real PUP is installed). **Not yet run on hardware** — next gate: sideload on Odin/Thor, iterate with adb logcat. **M2 complete on hardware (2026-08-05, AYN Thor / Adreno 740):** homebrew renders and takes input. Getting there needed four more patches — Adreno as its own `driver_vendor` (0009), a storage-buffer fallback for the runtime-sized UBO arrays upstream requires unconditionally (0011, the fix that turned a blank screen into pixels), and an embedder-provided pad handler hook (0012). Homebrew matrix on device: **10/10 valid PS3 payloads boot with zero fatal errors** — `gs_gcm_cube` renders a textured perspective cube with `cellFont` text, `gs_gcm_hello_world` renders text, `gs_gcm_tetris` renders its playfield, `spurs_test`/`rpcsp`/`ppu_thread`/`dump_stack`/`pad_test` run clean. (`pspgame.elf` is a 32-bit PSP binary, correctly rejected as "Invalid ELF class".) Controller input arrives through a new `android_pad_handler` fed by Android `KeyEvent`/`MotionEvent`. **M2 tail complete (2026-08-06): AdrenoTools driver loading shipped** — the bridge routes the core's Vulkan imports through generated arm64 trampolines (`native/bridge/vulkan_trampolines.S`, regenerated by `ci/gen-vk-trampolines.sh`) filled from either the system loader or an adrenotools-hooked one; drivers install in-app as adrenotools zips. Verified on the Thor with Mesa Turnip 26.0-rc8: correct rendering, ~15% faster than the stock blob in a heavy disc title (14.1–15.2 vs 11.9–13.4 FPS), and ~3× lower driver-side GPU memory (1.1 GB vs 3.1 GB) — Turnip is now the reference driver, as planned.

**M3 — Usable app.** *Partly done (2026-08-05):* `MANAGE_EXTERNAL_STORAGE` with an in-app grant prompt and a `GameLibrary` that scans the conventional roots for the three bootable shapes (disc images, `PS3_GAME`/`USRDIR` folders, bare homebrew). Audio works on device after patch 0013. Still open: Compose UI proper (setup wizard, settings subset, quick menu) and the first tagged APK release (arm64-v8a only).

**M4 — Frontend citizen.** *Partly done (2026-08-05):* the documented contract is now implemented, not just written — `bootPath` as path or URI, SAF `content://` document URIs mapped back to real paths, `ACTION_VIEW` (with a manifest filter), and `gameDir` for folder-format games. Still open: CocoonFE platform PR (+ Daijishō), savestates in quick menu, per-game settings.

**M5 — Fit and finish.** *Started (2026-08-05):* on-screen controller landed — d-pad, face buttons, shoulders, Select/Start, per-pointer multi-touch, three modes (Auto, which retires it once a physical pad is used; Always; Off) with a toggle in the UI. Perf passes on 8 Gen 2 vs Odin 3 (blob vs Turnip matrix), compatibility list (community reports template), SAF storage v2, touch overlay, submodule-bump cadence proven (≥2 bumps merged), upstream any patches that qualify.
