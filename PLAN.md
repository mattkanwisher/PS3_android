...
**M2 — Pixels and inputs.** *In progress (2026-08-05): first debug APK builds in CI* — `libcellstation.so` (JNI bridge + EmuCallbacks over the unmodified core, Vulkan surface via `ANativeWindow`, pad_thread, Cubeb/AAudio, lean PUP installer, logcat logging) packaged by a minimal Kotlin app (firmware install, game list from app-external storage, fullscreen `EmulationActivity` already exposing the public `EMULATE` intent). **Not yet run on hardware** — next gate: sideload on Odin/Thor, iterate with adb logcat. Remaining M2: on-device boot bring-up, Android pad handler for built-in controls, AdrenoTools driver loading.
